...
const elementoCabeca = document.getElementById('cabeca');
const elementoBracoD = document.getElementById('bracoD');
const elementoBracoE = document.getElementById('bracoE');
const elementoPeito = document.getElementById('peito');

function obterDimensoesNaturais() {
	const rectCabeca = elementoCabeca.getBoundingClientRect();

	// Aplicar a altura mais a margem superior ao braço D

	elementoBracoD.style.position = 'absolute';
	elementoBracoD.style.left = (rectCabeca.left * -6) + 'px';
	elementoBracoD.style.top = (elementoCabeca.offsetTop * 1.3) + 'px';
	elementoBracoD.style.width = (rectCabeca.width * 2) + 'px';

	elementoBracoE.style.position = 'absolute';
	elementoBracoE.style.left = (rectCabeca.left * 1.3) + 'px';
	elementoBracoE.style.top = (elementoCabeca.offsetTop * 1.3) + 'px';
	elementoBracoE.style.width = (rectCabeca.width * 2) + 'px';

	elementoPeito.style.position = 'absolute';
	elementoPeito.style.left = (rectCabeca.left * 1.1) + 'px';

	if (document.documentElement.clientWidth < 1350 && document.documentElement.clientWidth > 950) {
		elementoPeito.style.top = (elementoCabeca.offsetTop * 2.15) + 'px';
	} else if (document.documentElement.clientWidth < 950 && document.documentElement.clientWidth > 850) {
		elementoPeito.style.top = (elementoCabeca.offsetTop * 2.05) + 'px';
	} else if (document.documentElement.clientWidth < 850) {
		elementoPeito.style.top = (elementoCabeca.offsetTop * 2) + 'px';
	} else {
		elementoPeito.style.top = (elementoCabeca.offsetTop * 2.2) + 'px';
	}
	elementoPeito.style.width = (rectCabeca.width * 1.0) + 'px';
}

// Adicionar um manipulador de eventos para detectar alterações nas dimensões
window.addEventListener('resize', obterDimensoesNaturais);
obterDimensoesNaturais();
